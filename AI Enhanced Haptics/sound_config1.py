...
rumble_sound = pygame.mixer.Sound('rumble.wav')  # Load the sound file
...
def fill_tank(pressure):
    ...
    if current_tank_fill > 5000:
        secondary_increment = (pressure * current_tank_fill / 255) / 12  # Example rate: 10% of the overflow above 5000
        if pressure == 0:
...
def update_sound_volume(current_fill, max_capacity):
    volume = current_fill / max_capacity  # Normalize the volume between 0 and 1
    rumble_sound.set_volume(volume**2)  # Set the sound volume
...
def update_pressure():
    while not thread_stop.is_set():
        ...
        update_sound_volume(current_tank_fill, max_tank_capacity)
        rumble_sound.play(loops=-1)  # Loop the sound continuously
