...
def update_gauge(psi_value):
    ...
    if 7000 <= psi_value <= 7500:
        ...
        counter += 1  # Increment counter when in the optimal range
    else:
        status_label.config(text="Bad", fg="red")
        counter = 0  # Reset counter if out of the optimal range

    if counter >= 5:  # If the counter reaches 5, close the application
        root.destroy()
...
def update_pressure():
    ...
    while True:
        ...
        time.sleep(0.1)  # Check the pressure more frequently (every 0.1 seconds)
